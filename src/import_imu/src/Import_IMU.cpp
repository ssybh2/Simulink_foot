...
//
// Output and update for atomic system:
//    '<S11>/Motor_Target_Velocity_LA'
//    '<S11>/Motor_Target_Velocity_LB'
...
//    '<S15>/Motor_Position_To_Joint'
//    '<S16>/Motor_Position_To_Joint'
//
...
void Import_IMU::step()
{
  ...
  // End of MATLAB Function: '<S27>/RC_Safety_Gate'

  // MATLAB Function: '<S2>/RC_right_y_to_velocity' incorporates:
  //   MATLAB Function: '<Root>/Leg_X_Compensation'
  //   MATLAB Function: '<Root>/Yaw_Rate_Controller'
  //   MATLAB Function: '<S2>/RC_right_x_to_roll_command'

  Import_IMU_B.rightY_s = Import_IMU_B.In1_c.right_y;
  Import_IMU_B.vCmd = 0.0F;
  rtb_valid_k = !enable_safe;
  if ((!rtb_valid_k) && ((!rtIsInfF(Import_IMU_B.In1_c.right_y)) && (!rtIsNaNF
    ...
    Import_IMU_B.x = static_cast<real32_T>(fabs(static_cast<real_T>
      (Import_IMU_B.rightY_s)));
    if (Import_IMU_B.x <= 0.08F) {
  ...
  b_varargout_1 = rtIsInfF(Import_IMU_B.rightY_s);
  ...
  wheelEnable = (enable_safe && (rtb_valid_n && ((!wheelEnable_tmp_0) &&
    ...
    ((!b_varargout_1) && (!wheelEnable_tmp) && ((!rtIsInfF
    ...
    static_cast<real32_T>(Import_IMU_P.Constant5_Value_j))) && (!rtIsNaNF(
    static_cast<real32_T>(Import_IMU_P.Constant5_Value_j))) && ((!rtIsInfF(
    static_cast<real32_T>(Import_IMU_P.KdVelocity_Value))) && (!rtIsNaNF(
    static_cast<real32_T>(Import_IMU_P.KdVelocity_Value))) && ((!rtIsInfF(
    ...
    ((static_cast<real32_T>(Import_IMU_P.Constant5_Value_j) >= 0.0F) && (
  ...
  if (!wheelEnable) {
    ...
    Import_IMU_DW.velocityIntegralTerm = 0.0F;
    Import_IMU_B.w = 0.0F;
  } else {
    ...
    Import_IMU_B.w = Import_IMU_B.rightY_s - Import_IMU_B.relativeY;
    Import_IMU_B.relativeX = static_cast<real32_T>(Import_IMU_P.KpVelocity_Value)
      * Import_IMU_B.w;
    Import_IMU_B.relativeY *= -static_cast<real32_T>
      (Import_IMU_P.KdVelocity_Value);
    Import_IMU_B.x = static_cast<real32_T>(Import_IMU_P.Constant5_Value_j) *
      Import_IMU_B.w * 0.003F + Import_IMU_DW.velocityIntegralTerm;
    if (Import_IMU_B.x > 0.05F) {
      Import_IMU_B.x = 0.05F;
    } else if (Import_IMU_B.x < -0.05F) {
      Import_IMU_B.x = -0.05F;
    }

    Import_IMU_B.y = (Import_IMU_B.relativeX + Import_IMU_B.x) +
      Import_IMU_B.relativeY;
    if (Import_IMU_B.y > 1.0F) {
      tmp = (Import_IMU_B.w < 0.0F);
    } else {
      tmp = ((Import_IMU_B.y < -1.0F) && (Import_IMU_B.w > 0.0F));
    }

    if (((Import_IMU_B.y <= 1.0F) && (Import_IMU_B.y >= -1.0F)) || tmp) {
      Import_IMU_DW.velocityIntegralTerm = Import_IMU_B.x;
    }

    Import_IMU_B.relativeX = (Import_IMU_B.relativeX +
      Import_IMU_DW.velocityIntegralTerm) + Import_IMU_B.relativeY;
  ...
  Import_IMU_B.y = 0.0F;
  tmp = (rtb_valid_k || (!rtb_valid_n));
  if ((!tmp) && ((!rtIsInfF(Import_IMU_B.relativeX)) && (!rtIsNaNF
  ...
  if (rtb_valid_k || (rtIsInfF(Import_IMU_B.w) || rtIsNaNF(Import_IMU_B.w) ||
    ...
    Import_IMU_B.xVelocityComp = 0.0F;
  } else {
  ...
  // MATLAB Function: '<S15>/Motor_Position_To_Joint' incorporates:
  //   Constant: '<S15>/qj0'
  //   Constant: '<S15>/qm0'
  //   Constant: '<S15>/ratio'
  //   Constant: '<S15>/sign'
  ...
  // MATLAB Function: '<S16>/Motor_Position_To_Joint' incorporates:
  //   Constant: '<S16>/qj0'
  //   Constant: '<S16>/qm0'
  //   Constant: '<S16>/ratio'
  //   Constant: '<S16>/sign'
  ...
  Import_IMU_FiveBar_FK_Left(Import_IMU_B.y, Import_IMU_B.z, Import_IMU_P.L1,
    ...
    &Import_IMU_B.leg_angle, &rtb_valid_n);

  // MATLAB Function: '<S2>/RC_right_x_to_roll_command' incorporates:
  //   Constant: '<S2>/Constant'
  //   Constant: '<S2>/Constant1'

  Import_IMU_B.leg_length = static_cast<real32_T>(fabs(static_cast<real_T>(
    static_cast<real32_T>(Import_IMU_P.Constant_Value_fu))));
  if (rtb_valid_k || (rtIsInfF(Import_IMU_B.In1_c.right_x) || rtIsNaNF
                      (Import_IMU_B.In1_c.right_x) || (rtIsInfF
        (Import_IMU_B.leg_length) || rtIsNaNF(Import_IMU_B.leg_length) ||
        (rtIsInfF(static_cast<real32_T>(Import_IMU_P.Constant1_Value)) ||
         rtIsNaNF(static_cast<real32_T>(Import_IMU_P.Constant1_Value)))))) {
    Import_IMU_DW.rollCmdFiltered = 0.0F;
    Import_IMU_DW.previousEnable_c = false;
    Import_IMU_B.rollCmd = 0.0F;
  } else {
    if (!Import_IMU_DW.previousEnable_c) {
      Import_IMU_DW.rollCmdFiltered = 0.0F;
    }

    Import_IMU_DW.previousEnable_c = true;
    if (Import_IMU_B.In1_c.right_x > 1.0F) {
      Import_IMU_B.leg_angle = 1.0F;
    } else if (Import_IMU_B.In1_c.right_x < -1.0F) {
      Import_IMU_B.leg_angle = -1.0F;
    } else {
      Import_IMU_B.leg_angle = Import_IMU_B.In1_c.right_x;
    }

    Import_IMU_B.stickMagnitude = static_cast<real32_T>(fabs(static_cast<real_T>
      (Import_IMU_B.leg_angle)));
    if (Import_IMU_B.stickMagnitude <= 0.08F) {
    ...
    if (Import_IMU_B.leg_angle > 0.0024F) {
      Import_IMU_B.leg_angle = 0.0024F;
    } else if (Import_IMU_B.leg_angle < -0.0024F) {
      Import_IMU_B.leg_angle = -0.0024F;
    }

    Import_IMU_DW.rollCmdFiltered += Import_IMU_B.leg_angle;
    if (Import_IMU_DW.rollCmdFiltered > Import_IMU_B.leg_length) {
      Import_IMU_DW.rollCmdFiltered = Import_IMU_B.leg_length;
    } else if (Import_IMU_DW.rollCmdFiltered < -Import_IMU_B.leg_length) {
      Import_IMU_DW.rollCmdFiltered = -Import_IMU_B.leg_length;
    }

    Import_IMU_B.rollCmd = Import_IMU_DW.rollCmdFiltered;
  }
  ...
  if (tmp || (rtIsInf(Import_IMU_B.Switch1_g) || rtIsNaN(Import_IMU_B.Switch1_g)
              || (rtIsInfF(Import_IMU_B.xB_c) || rtIsNaNF(Import_IMU_B.xB_c) ||
                  (rtIsInfF(Import_IMU_B.yB_o) || rtIsNaNF(Import_IMU_B.yB_o) ||
                   (rtIsInfF(Import_IMU_B.xB) || rtIsNaNF(Import_IMU_B.xB) ||
                    (rtIsInfF(Import_IMU_B.yB) || rtIsNaNF(Import_IMU_B.yB) ||
                     (rtIsInfF(Import_IMU_B.deltaYRollRaw) || rtIsNaNF
                      (Import_IMU_B.deltaYRollRaw) || (rtIsInfF
  ...
  //   DataTypeConversion: '<S11>/Data Type Conversion46'
  //   DataTypeConversion: '<S11>/Data Type Conversion47'
  //   DataTypeConversion: '<S11>/Data Type Conversion48'
  //   MATLAB Function: '<S11>/Left_Joint_To_Motor_L'
  ...
  //   Constant: '<Root>/Constant27'
  //   DataTypeConversion: '<S11>/Data Type Conversion'
  //   DataTypeConversion: '<S11>/Data Type Conversion42'
  //   DataTypeConversion: '<S11>/Data Type Conversion46'
  //   DataTypeConversion: '<S11>/Data Type Conversion47'
